create index if not exists workout_sessions_client_date_idx
  on public.workout_sessions (client_id, date desc);
create index if not exists workout_sessions_routine_id_idx
  on public.workout_sessions (routine_id);
create index if not exists workout_session_sets_session_id_idx
...
alter table public.workout_session_sets
  add constraint workout_session_sets_set_number_check check (set_number > 0),
  add constraint workout_session_sets_reps_check check (reps is null or reps > 0),
  add constraint workout_session_sets_weight_check check (weight is null or weight >= 0),
  add constraint workout_session_sets_unique_set unique (
    workout_session_id,
    exercise_id,
    set_number
  );
...
begin
  ...
  insert into public.workout_session_sets (
    workout_session_id, exercise_id, set_number, reps, weight, completed
  )
  select
    session_id,
